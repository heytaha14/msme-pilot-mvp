  ...
  return (
    <div
      className={clsx(
        ...
        typeToneMap[type] || 'bg-slate-100 text-slate-600 ring-slate-200',
      )}
    >
...
function BulkActionBar({ count, onArchive, onDelete, onMarkRead }) {
  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-start gap-4">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <Badge variant={getNotificationPriorityBadge(notification.priority)}>
              {notification.priority}
            </Badge>
            <Badge variant={getNotificationStatusBadge(notification.status)}>
              {notification.status}
            </Badge>
      ...
      <div className="flex flex-wrap items-center gap-2 lg:justify-end">
        <Button onClick={() => onTakeAction(notification)} size="sm" variant="secondary">
    ...
    <Card className={clsx(isUnread && 'border-indigo-100 bg-indigo-50/40')} hover>
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <NotificationIcon type={notification.type} />
          <div className="min-w-0">
            <h3 className="font-black leading-5 text-slate-950">
              {notification.title}
            </h3>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2">
        <Button className="col-span-2" onClick={() => onTakeAction(notification)} size="sm" variant="secondary">
          {notification.actionLabel}
        </Button>
        <Button onClick={() => onToggleRead(notification)} size="sm" variant="ghost">
          {isUnread ? 'Mark read' : 'Mark unread'}
        </Button>
        <Button onClick={() => onArchive(notification.id)} size="sm" variant="ghost">
          Archive
        </Button>
        <Button
...
function NotificationInsights() {
  return (
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {notificationInsights.map((insight, index) => {
...
function ToggleRow({ checked, label, onToggle }) {
  return (
    <button
      className="flex w-full items-center justify-between rounded-2xl border border-slate-200 bg-white p-4 text-left transition hover:border-indigo-200 hover:bg-indigo-50/40"
      onClick={onToggle}
      type="button"
    >
    ...
    <ModalShell onClose={onClose}>
      <Card className="max-h-[92vh] overflow-y-auto" padding="lg">
        <div className="flex items-start justify-between gap-4">
          <div>
            <Badge variant="info">Demo preferences</Badge>
            <h2 className="mt-3 text-2xl font-black text-slate-950">
              Notification Settings
            </h2>
            <p className="mt-2 text-sm leading-6 text-slate-500">
              Control which alerts appear in-app. Email, WhatsApp, and push delivery
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {notificationSettings.map((setting) => (
            <ToggleRow
              checked={preferenceState[setting]}
              key={setting}
              label={setting}
              onToggle={() => onTogglePreference(setting)}
            />
          ))}
        </div>
...
export default function NotificationsPage() {
  ...
  const [preferenceState, setPreferenceState] = useState(() =>
    Object.fromEntries(notificationSettings.map((setting) => [setting, true])),
  );
  const [channelState, setChannelState] = useState({
    'In-app': true,
    Email: false,
    'WhatsApp later': false,
  });
  ...
  const filteredNotifications = useMemo(() => {
    ...
    return notifications.filter((notification) => {
      const matchesSearch =
        !searchTerm ||
        [notification.title, notification.message, notification.relatedEntity]
          .join(' ')
          .toLowerCase()
          .includes(searchTerm);
      const matchesType =
  ...
  const updateNotification = (id, updater) => {
    setNotifications((current) =>
      current.map((notification) =>
        notification.id === id ? updater(notification) : notification,
      ),
    );
  };
  ...
  const archiveNotification = (id) => {
    ...
    showSuccess('Notification archived locally.');
  };
  ...
  const deleteNotification = (id) => {
    ...
    setSelectedIds((current) => current.filter((selectedId) => selectedId !== id));
    showSuccess('Notification deleted locally.');
  };
  ...
  const bulkMarkRead = () => {
    ...
    setSelectedIds([]);
    showSuccess('Selected notifications marked as read.');
  };
  ...
  const bulkArchive = () => {
    ...
    setSelectedIds([]);
    showSuccess('Selected notifications archived locally.');
  };
  ...
  const bulkDelete = () => {
    ...
    setSelectedIds([]);
    showSuccess('Selected notifications deleted locally.');
  };

  const takeAction = () => {
    showSuccess('Action opened in demo mode.');
  };
  ...
  const summaryCards = [
    {
      title: 'Total Alerts',
      value: '24',
      trend: 'Across stock, invoices, dues, and health',
      icon: Bell,
      status: 'info',
    },
    {
      ...
      trend: unreadCount ? 'Needs owner attention' : 'All caught up',
      icon: BellDot,
      status: unreadCount ? 'warning' : 'success',
    },
    {
      title: 'Critical',
      value: '3',
      trend: 'Sugar, overdue supplier, cash flow',
      icon: TriangleAlert,
      status: 'danger',
    },
    {
      title: 'Due Today',
      value: '5',
      trend: 'Actions to review before closing',
      icon: Clock,
      status: 'neutral',
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
            Track stock alerts, payment reminders, invoice updates, and business
            warnings.
          </p>
        </div>
        ...
        <div className="relative grid gap-6 lg:grid-cols-[auto_1fr_auto] lg:items-center">
          ...
          <div>
            ...
            <h2 className="mt-3 text-2xl font-black tracking-tight">
              Restock, recover, and review first.
            </h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
              Your most urgent actions today are to restock Sugar, recover ₹12,000
              from Ahmed Traders, and review 5 pending invoices.
            </p>
          </div>
          <Button onClick={() => showSuccess('Action plan opened in demo mode.')} variant="secondary">
      ...
      <SectionHeader
        subtitle={`${filteredNotifications.length} alerts shown. Manage read status, archive items, or open demo actions.`}
      ...
      <SectionHeader
        subtitle="A compact view of what needs attention across the business."
      ...
      {!notifications.length ? (
        <Card className="py-12 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            New stock, payment, invoice, and health alerts will appear here.
          </p>
        </Card>
      ) : null}
